ALTER TYPE "public"."job_phase" ADD VALUE 'parsing' BEFORE 'embedding';--> statement-breakpoint
ALTER TYPE "public"."source_status" ADD VALUE 'cancelled';--> statement-breakpoint
...
- Never answer a request to reach a person with unrelated page content.';--> statement-breakpoint
ALTER TABLE "crawl_jobs" ADD COLUMN "batch_id" uuid;--> statement-breakpoint
ALTER TABLE "crawl_jobs" ADD COLUMN "cancel_requested_at" timestamp with time zone;--> statement-breakpoint
CREATE INDEX "crawl_jobs_batch_idx" ON "crawl_jobs" USING btree ("batch_id");